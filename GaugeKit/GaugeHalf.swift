...
protocol GaugeHalf {
    func getVerticalHalfGauge(_ rotatengle: Double) -> CAShapeLayer
    func getHorizontalHalfGauge(_ rotatengle: Double) -> CAShapeLayer
}

extension Gauge: GaugeHalf {
    func getVerticalHalfGauge(_ rotateAngle: Double) -> CAShapeLayer {
    ...
    func getHorizontalHalfGauge(_ rotateAngle: Double) -> CAShapeLayer {
        let gaugeLayer = CAShapeLayer()

        //        let rotatedBounds = CGRectMake(10, 10, bounds.height, bounds.width)
        let newBounds = CGRect(x: bounds.origin.x, y: bounds.origin.y, width: bounds.width, height: bounds.height * 2 - lineWidth)

        //        var newBounds = bounds
        if bgLayer == nil {
            bgLayer = CAShapeLayer.getOval(lineWidth, strokeStart: 0, strokeEnd: 0.5, strokeColor: _bgStartColor,
                                           fillColor: UIColor.clear, shadowRadius: shadowRadius, shadowOpacity: shadowOpacity, shadowOffsset: CGSize.zero, bounds: newBounds, rotateAngle: 0, isCircle: isCircle)
            bgLayer.frame = layer.bounds
           // bgLayer.position = CGPoint(x: bgLayer.position.x + bounds.width - lineWidth, y: bgLayer.position.y)
            bgLayer.position = CGPoint(x: bgLayer.position.x , y: bgLayer.position.y  - bounds.height + lineWidth)
        }
        ...
        if ringLayer == nil {
            ringLayer = CAShapeLayer.getOval(lineWidth, strokeStart: 0, strokeEnd: 0.5, strokeColor: startColor,
                                             fillColor: UIColor.clear, shadowRadius: shadowRadius, shadowOpacity: shadowOpacity, shadowOffsset: CGSize.zero, bounds: newBounds, rotateAngle: 0, isCircle: isCircle)
            ringLayer.frame = layer.bounds
            ringLayer.position = CGPoint(x: ringLayer.position.x , y: ringLayer.position.y  - bounds.height + lineWidth)
        }
        ...
        gaugeLayer.frame = layer.bounds
        gaugeLayer.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        gaugeLayer.transform = CATransform3DRotate(gaugeLayer.transform, CGFloat(0), 0, 0, 1)
        if reverse {
            reverseX(gaugeLayer)
        }
